...
<div class="row">
	<div class="col-lg-12">
		<section class="panel">
			{!! Form::open(['route' => [$route.'.update', $stock->id], 'method' => 'PUT', 'class' => 'form-horizontal form-bordered']) !!}
				<header class="panel-heading">
					<div class="panel-actions">
						<a href="#" class="panel-action panel-action-toggle" data-panel-toggle=""></a>
					</div>

					<h2 class="panel-title">แก้ไขข้อมูลสินค้า</h2>
				</header>
				<div class="panel-body">
						...
						<div class="col-md-3">
							<select data-plugin-selectTwo name="product_id" id="product_id" class="form-control populate" required>
								@foreach($productList as $product)
									<option value="{{ $product->id }}"{!! old('product_id', $stock->product->id) == $product->id ? ' selected="selected"' : '' !!}>{{ $product->name }}</option>
								@endforeach
							</select>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-3">
							<input type="text" class="form-control" name="expire_date" id="expire_date" style="cursor: pointer" value="{{ old('expire_date', date('Y-m-d', strtotime($stock->expire_date))) }}" readonly required>
						</div>
					</div>
